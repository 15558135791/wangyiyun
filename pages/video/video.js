...
Page({
  ...
  async getListDetail(navId) {
    ...
    //   let result = request('/video/url', { id: item.id }).then(r => {
    //     item.videoUrl = r.urls[0].url
    //   })
    // })
    this.setData({
  ...
  //   let index = 0;
  //   let videoList = videoListData.datas.map(
  //     item => {
  //       item.id = index++;
  //       return item;
  //     })
  ...
  handlePlay(event) {
    ...
    问题：多个视频同时播放的问题
      需求：
        ...
        2.在播放的新的视频之前关闭上一个正在播放的视频
      关键:
        1.如何找到上一个视频的实例对象
        2.如何确认点击播放的视频和正在播放的视频不是同一个视频
      单例模式：
         1.需要创建多个对象的场景下，通过一个变量接受，始终保持一个对象
         2.节省内存空间
    */
